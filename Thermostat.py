...
class TemperatureMachine(StateMachine):
    ...
    def on_exit_heat(self):
        # turn both LEDs off when exiting heat
        redLight.off()
        blueLight.off()
        if DEBUG:
            print("* Exiting heat state")
    ...
    def on_exit_cool(self):
        # turn both LEDs off when exiting cool
        redLight.off()
        blueLight.off()
        if DEBUG:
    ...
    def processTempIncButton(self):
        ...
        self.setPoint += 1      # bump the set point up one degree
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        self.setPoint -= 1      # bump the set point down one degree
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if self.current_state.id == 'heat':
            ...
            if temp < self.setPoint:
                    redLight.pulse()    # fade in/out when below set‑point
            else:
                    redLight.on()       # solid on at or above set‑point

        elif self.current_state.id == 'cool':
            # cooling
            if temp > self.setPoint:
                blueLight.pulse()   # fade in/out when above set‑point
            else:
                blueLight.on()      # solid on at or below set‑point

        if DEBUG:
            print(f"updateLights → state={self.current_state.id}, temp={temp}, setPoint={self.setPoint}")
    ...
    def setupSerialOutput(self):
        ...
        ## Remove this TODO comment block when complete.
        # grab the current temp as an integer
        temp = floor(self.getFahrenheit())
        # build a comma‑delimited string: state,temp,setPoint
